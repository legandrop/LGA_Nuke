...
update Lega mod v1.54   - Text input box that asks for the backdrop name at the moment of creation 
...
def autoBackdrop(show_input=True):
    ...
    # Imprimir valores de depuracion antes de ajustar bdX, bdY, bdW, bdH
    ...
    # Imprimir valores de depuracion despues de ajustar bdX, bdY, bdW, bdH
...
# Verificar si ya existe un atributo de usuario para almacenar el color
if not node.knob('current_text_color'):
    color_knob = nuke.String_Knob('current_text_color', 'Current Text Color', '#ffffff')
    color_knob.setVisible(False)
    node.addKnob(color_knob)
else:
    color_knob = node['current_text_color']

# Verificar si el texto esta en bold
is_bold = '<b>' in current_label and '</b>' in current_label

# Recuperar el alineamiento actual
alignment_knob = node.knob('alignment')
alignment = alignment_knob.value() if alignment_knob else 'left'  # Valor por defecto: 'left'

# Alternar entre blanco y negro
if 'color="#ffffff"' in current_label or color_knob.value() == '#ffffff':
    new_color = '#000000'
else:
    new_color = '#ffffff'

# Preservar el formato bold y el alineamiento
if is_bold:
    new_label = f'<p align="{alignment}"><font color="{new_color}"><b>{node["text"].value()}</b></font></p>'
else:
    new_label = f'<p align="{alignment}"><font color="{new_color}">{node["text"].value()}</font></p>'

# Actualizar el label con el nuevo color y formato
node['label'].setValue(new_label)
color_knob.setValue(new_color)
''')
...
def on_knob_changed():
    ...
    if not node.knob('current_text_color'):
        # Si no existe el knob que almacena el color actual, crearlo y asignar blanco por defecto
        color_knob = nuke.String_Knob('current_text_color', 'Current Text Color', '#ffffff')
        color_knob.setVisible(False)
        node.addKnob(color_knob)
    else:
    ...
    current_color = color_knob.value()

    if knob.name() in ["text", "Oz_Backdrop_bold", "alignment", "font_size"]:
        if node.knob("text") and node.knob("Oz_Backdrop_bold"):
            ...
            formatted_label = f'<font color="{current_color}">{formatted_label}</font>'
        ...
        # Mantener el color actual del texto
        current_label = f'<font color="{current_color}">{current_label}</font>'
