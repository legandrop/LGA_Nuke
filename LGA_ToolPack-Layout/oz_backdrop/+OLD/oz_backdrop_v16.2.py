...
update Lega mod v1.62   - Text input box that asks for the backdrop name at the moment of creation 
...
def order_overlapping_backdrops(new_backdrop):
    ...
    print(f"Total de backdrops: {len(all_backdrops)}")
    ...
    print(f"Backdrops superpuestos: {len(overlapping_group)}")
    
    # Si no hay superposiciones, no es necesario hacer nada
    if len(overlapping_group) == 1:
        print("No hay superposiciones, saliendo.")
        return
    ...
    print("--- Finalizando order_overlapping_backdrops ---\n")
