...
# Chequea si ya hay un color definido, si no, lo define.
...
def on_knob_changed():
    ...
    # Obtener el color actual desde la propiedad personalizada
    font_color = node.knob('Oz_Font_Color').value() if node.knob('Oz_Font_Color') else '#ffffff'
    ...
    if knob.name() == "text" or knob.name() == "Oz_Backdrop_bold":
        if node.knob("text") and node.knob("Oz_Backdrop_bold"):
            ...
            formatted_label = f'<font color="{font_color}">' + formatted_label + '</font>'
        ...
        current_label = f'<font color="{font_color}">' + current_label + '</font>'
