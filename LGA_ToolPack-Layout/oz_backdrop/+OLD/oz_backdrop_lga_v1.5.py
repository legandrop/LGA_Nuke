...
def replaceOldBackdrops():
    ...
    for bd_node in bd_nodes:
        ...
        # Aplicar el valor de negrita si esta presente en el nodo antiguo
        if bd_node.knob('Oz_Backdrop_bold'):
            new_bd['Oz_Backdrop_bold'].setValue(bd_node['Oz_Backdrop_bold'].value())
            # Asegurarse de aplicar la negrita al texto si esta activado
            if new_bd['Oz_Backdrop_bold'].value():
                current_text = new_bd['text'].value()
                new_bd['label'].setValue(f"<b>{current_text}</b>")
