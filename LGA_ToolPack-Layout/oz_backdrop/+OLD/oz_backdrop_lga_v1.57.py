...
print(f"Boton B/W: current_label antes de cualquier cambio: {current_label}")

# Verifica si el knob 'Oz_Font_Color' existe, si no, lo crea.
if 'Oz_Font_Color' not in node.knobs():
    print("Boton B/W: Creando knob Oz_Font_Color")
    font_color_knob = nuke.String_Knob("Oz_Font_Color", "Font Color")
    node.addKnob(font_color_knob)
    node['Oz_Font_Color'].setValue("#ffffff")  # Establece el valor por defecto a blanco
else:
    print(f"Boton B/W: Knob Oz_Font_Color ya existe con valor: {node['Oz_Font_Color'].value()}")

# Chequea si ya hay un color definido en la etiqueta, si no, lo define.
if 'color="#ffffff"' in current_label:
    print("Boton B/W: Se detecta color blanco en la etiqueta, cambiando a negro")
    new_label = current_label.replace('color="#ffffff"', 'color="#000000"')
    node['Oz_Font_Color'].setValue("#000000")
elif 'color="#000000"' in current_label:
    print("Boton B/W: Se detecta color negro en la etiqueta, cambiando a blanco")
    new_label = current_label.replace('color="#000000"', 'color="#ffffff"')
    node['Oz_Font_Color'].setValue("#ffffff")
else:
    print("Boton B/W: No se detecta color, anadiendo blanco por defecto")
    new_label = '<font color="#ffffff">' + current_label + '</font>'
    node['Oz_Font_Color'].setValue("#ffffff")

print(f"Boton B/W: new_label despues de cambios: {new_label}")
