    ...
    try:
        ...
        if len(nodes) > 1:
            ...
            if same_input_node is not None:
                debug_print_A("All selected nodes have at least one input connected to the same node:", same_input_node['name'].value())
                ...
                if same_input_node.Class() == "Dot":
                    ...
                    dot_node.setXYpos(same_input_node.xpos() + same_input_node.screenWidth() // 2 - dsize // 2, same_input_node.ypos() + same_input_node.screenHeight() + 50)
                    ...
                    debug_print_B("A Dot has been created after the node", same_input_node['name'].value(), "and all selected nodes have been connected to it")
                ...
                for node, x_center in node_positions:
                    if abs(x_center - dot_x_center) <= tolerance_x:
                        position_labels[node['name'].value()] = 0  # Consider as in the same column
                ...
                count_left = -1
                for node, _ in reversed(left_positions):  # Reversed to start from the closest to the Dot
                ...
                for node_name, position in position_labels.items():
                    for node, x_center in node_positions:
                        if node['name'].value() == node_name:
                            debug_print_B("Node:", node_name, "Relative position to the Dot:", position, "Centered X position:", x_center)
                            ...
                            for node, x_center in node_positions:
                                if node['name'].value() == node_name:
                                    if current_position == -1:
                                        previous_dot = mainDots(node, common_node=same_input_node, previous_dot=None)  # Pass the common node and previous_dot as None
                                        debug_print_B(f"Node {node_name} in position -1 connected to Dot: {previous_dot['name'].value() if previous_dot else 'None'}")
                                    else:
                                        # Connect the current node to the Dot of the previous node
                                        node.setInput(0, previous_dot)
                                        previous_dot = mainDots(node, common_node=same_input_node, previous_dot=previous_dot)  # Pass the common node and the updated previous_dot
                                        debug_print_B(f"Node {node_name} in position {current_position} connected to Dot: {previous_dot['name'].value() if previous_dot else 'None'}")
                ...
                debug_print_B("Not all selected nodes have inputs connected to the same node. Processing each selected node individually")
                for selected in nodes:
        ...
        # Check if only one node is selected
            ...
            multiple_inputs = any(selected_node.input(i) is not None for i in range(1, selected_node.maxInputs()))
            ...
            if connected_node:
                ...
                if abs(selected_x_center - connected_x_center) <= tolerance_x and not multiple_inputs:
                    if not moveSelectedNode:
                        debug_print_B(f"The node {selected_node['name'].value()} is in the same column as its connected node {connected_node['name'].value()}, does not have multiple inputs, and moveSelectedNode is False. Nothing will be done.")
                    else:
                        selected_node.setXpos(connected_node.xpos() + connected_node.screenWidth() // 2 - selected_node.screenWidth() // 2)
                        debug_print_B(f"The node {selected_node['name'].value()} has been moved to align with the X position of its connected node {connected_node['name'].value()}.")
                else:
                    debug_print_B(f"The node {selected_node['name'].value()} is NOT in the same column as its connected node {connected_node['name'].value()} or has multiple inputs. Executing mainDots().")
                    mainDots(selected_node, common_node=None, previous_dot=None)  # Pass None if there is no common node and previous_dot as None
            else:
                debug_print_B(f"The node {selected_node['name'].value()} has no connected node. Executing mainDots().")
                mainDots(selected_node, common_node=None, previous_dot=None)  # Pass None if there is no common node and previous_dot as None
...
def mainDots(selected, common_node=None, previous_dot=None):
    ...
    debug_print_A(f"    Inputs found: A={A['name'].value() if A else 'None'}, B={B['name'].value() if B else 'None'}, C={C['name'].value() if C else 'None'}")
    ...
    if common_node is not None or previous_dot is not None:
        for idx, input_node in enumerate([A, B, C]):
            if input_node == common_node or input_node == previous_dot:
                new_dot = process_input(input_node, selected, idx, tolerance_x, tolerance_y, dsize, moveSelectedNode, force_single_input=True)
        ...
        new_dot_A = process_input(A, selected, 0, tolerance_x, tolerance_y, dsize, moveSelectedNode)
        new_dot_B = process_input(B, selected, 1, tolerance_x, tolerance_y, dsize, moveSelectedNode)
        new_dot_C = process_input(C, selected, 2, tolerance_x, tolerance_y, dsize, moveSelectedNode)
...
def process_input(input_node, selected_node, index, tolerance_x, tolerance_y, dsize, moveSelectedNode, force_single_input=False):
    if input_node is not None:

        # Check if the class of the selected node starts with any of the names in the list
        isNodeInVvsH_Nodes = any(selected_node.Class().startswith(node) for node in VvsH_Nodes)

        if should_create_dot(input_node, selected_node, tolerance_x, tolerance_y, index, isNodeInVvsH_Nodes):
            dot = nuke.nodes.Dot()
            ...
            if (selected_node.input(0) is not None and selected_node.input(1) is None and selected_node.input(2) is None) or force_single_input:
                # Align the Dot in Y with the input node and in X with the selected node
                dot.setXYpos(int(selected_node.xpos() + selected_node.screenWidth() // 2 - dsize // 2),
                             int(input_node.ypos() + input_node.screenHeight() // 2 - dsize // 2))
                debug_print_A(f"        Dot created and aligned with input node {input_node['name'].value()} in Y and with selected node {selected_node['name'].value()} in X.")
            else:
                # Align the Dot in Y and X with the selected node
                dot.setXYpos(int(input_node.xpos() + input_node.screenWidth() // 2 - dsize // 2),
                             int(selected_node.ypos() + selected_node.screenHeight() // 2 - dsize // 2))
                debug_print_A(f"        Dot created and aligned with selected node {selected_node['name'].value()} in both X and Y.")
            ...
            if moveSelectedNode:
                ...
                if (isInTheSameColumn or isInVerticalException or isInHorizontalException) and distance_x != 0:
                    selected_node.setXpos(input_node.xpos() + input_node.screenWidth() // 2 - selected_node.screenWidth() // 2)
                    debug_print_A(f"        Selected node moved to the X position of {input_node['name'].value()}: {selected_node.xpos()} (original: {selected_x_center})")
...
def should_create_dot(input_node, selected_node, tolerance_x, tolerance_y, index, isNodeInVvsH_Nodes):
    ...
    if distance_x > tolerance_x:
        debug_print_A(f"            X of {distance_x} is greater than Tolerance X of {tolerance_x} - A Dot node might be created")
        X_OK = True
    else:
        debug_print_A(f"            X of {distance_x} is less than Tolerance X of {tolerance_x} - A Dot node will not be created")
        X_OK = False

    if distance_y > tolerance_y:
        debug_print_A(f"            Y of {distance_y} is greater than Tolerance Y of {tolerance_y} - A Dot node might be created")
        Y_OK = True
    else:
        debug_print_A(f"            Y of {distance_y} is less than or equal to Tolerance Y of {tolerance_y} - A Dot node will not be created")
        Y_OK = False
    ...
    if isInVerticalException:
        debug_print_A(f"            Verical Comp mode ON, on an important Vertical vs Horizontal Node while processing the input 0 (B input). Lets move the node instead of creating a dot")
        X_OK = False
    elif isInHorizontalException:
        debug_print_A(f"            Verical Comp mode OFF, on an important Vertical vs Horizontal Node while processing the input 1 (A input). Lets move the node instead of creating a dot")
        X_OK = False


    if X_OK and Y_OK:
        debug_print_A(f"            Distances within tolerance - A Dot will be created at node {input_node['name'].value()}")
        return True
