...
___________________________________________________________________________________________
  ...
  LGA_NodePack v1.3 - 2024 -  Lega Pugliese
...
def load_files_in_directory(directory, menu):
    list_files = sorted(os.listdir(directory), key=lambda x: os.path.splitext(x)[0].lower())
    directories = []
    files = []
    ...
    for file_name, file_path in files:
        ...
        if file_name != 'menu':
            if file_path.endswith('.gizmo'):
                menu.addCommand(file_name, 'nuke.createNode("{}")'.format(file_name), icon=f_icon_path)
            elif file_path.endswith('.nk') or (file_path.endswith('.py') and 'init' in file_name):
                menu.addCommand(file_name, 'nuke.loadToolset("{}")'.format(file_path.replace('\\', '/')), icon=f_icon_path)
...
subfolders = [os.path.join(SCRIPT_DIR, d) for d in sorted(os.listdir(SCRIPT_DIR), key=lambda x: x.lower()) if os.path.isdir(os.path.join(SCRIPT_DIR, d))]

# Crear menus para cada subcarpeta que no contenga *init*.py o menu.py
for folder in subfolders:
    menu_name = os.path.basename(folder)
    if not os.path.exists(os.path.join(folder, 'menu.py')) and not any(file.endswith('.py') and 'init' in file for file in os.listdir(folder)):
        sub_menu = nodes_menu.addMenu(menu_name, icon=get_folder_icon(menu_name, folder))
