...
_________________________________________________
  ...
  LGA_ReviewToolsPanel v2.5 - 2024 - Lega Pugliese
...
class ReconnectMediaWidget(QWidget):
    def __init__(self):
        ...
        self.buttons = [
            ("Self ReplaceClip", self.SelfReaplaceClip, "#0e1f3a"),
            ("ON Clips | OFF v00", self.enable_or_disable_clips, "#0e1f3a"), 
            ("EXR Track Difference", self.toggle_blend_mode_for_exr_track, "#283526"),
            ("Compare Versions", self.run_compare_versions, "#273c24"),
            ("Compare OFF", self.compare_versions_off, "#273c24"),
            ("&Viewer | Rec709", self.rec709_viewer, "#311840", "Shift+V", "Shift+V"),
            ("Viewer | 2.35:1 ", self.viewer_235, "#311840"),        
            ("Reveal in &Explorer", self.reveal_in_explorer, "#321a1a", "Shift+E", "Shift+E"),
            ("Reveal NKS Project", self.reveal_project, "#321a1a"),
            ("Reveal NK Sc&ript", self.reveal_script, "#321a1a", "Shift+R", "Shift+R"),
            ("OpenInNuke&X", self.open_in_nukex_main, "#493800", "Shift+X", "Shift+X")
        ]
    ...
    def move_clip_to_bin(self, project, clip_name, source_bin_name, target_bin_path, shot):
        ...
        if source_bin:
            ...
            if clip_to_move:
                ...
                debug_print("Se movio el clip '{}' del bin '{}' al bin '{}'.".format(clip_name, source_bin_name, target_bin_path))
            else:
                debug_print("No se encontro el clip '{}' en el bin de origen '{}'.".format(clip_name, source_bin_name))
        else:
            debug_print("No se encontro el bin de origen '{}'.".format(source_bin_name))
    ...
    def run_compare_versions(self):
        try:
            ...
            for selected_clip in selected_clips:
                ...
                # Comprobar si el clip esta en un track llamado EXR
                if selected_clip.parentTrack().name() != "EXR":
                    continue  # Ignorar si no esta en un track llamado EXR
                ...
                if os.path.exists(script_path):
                    ...
                    # Llamar a la funcion main del script y pasar el clip seleccionado
                    module.main(selected_clip)
                    return  # Salir despues de procesar el primer clip valido
                else:
