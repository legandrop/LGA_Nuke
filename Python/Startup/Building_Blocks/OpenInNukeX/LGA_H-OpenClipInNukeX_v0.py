...
def open_nuke_script(nk_filepath):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(10)
            s.connect((host, port))
            # Enviar un comando de prueba para confirmar que NukeX esta respondiendo
            test_command = "ping"  # Asegurate de que este comando sea algo que NukeX reconocera y respondera adecuadamente
            s.sendall(test_command.encode())
            # Esperar una respuesta para confirmar que NukeX esta operativo
            response = s.recv(1024).decode()
            if "pong" in response:  # Asume que NukeX respondera con 'pong' a 'ping'
                #print("NukeX is active and responding.")
                show_timed_message(
                    "OpenInNukeX", 
                    (
                        f"<div style='text-align: center;'>"
                        f"<span>Opening {script_name}</span><br><br>"
                        f"<span style='color:white;'>Please switch to the NukeX window...</span>"
                        f"</div>"
                    ),
                    5000
                )
                return  # NukeX esta activo y no es necesario hacer mas
            else:
