...
def get_shot_and_tasks(sg, shot_code):
    # Primero obtener solo la información básica del shot
    ...
    shot_data = sg.find_one("Shot", filters, fields)
    
    if not shot_data:
        return None
    
    # Obtener las tareas por separado, como en la v01
    task_filters = [['entity', 'is', {'type': 'Shot', 'id': shot_data['id']}]]
    task_fields = [
        'id',
        'content',
        'sg_status_list',
    ...
    shot_data['tasks'] = sg.find("Task", task_filters, task_fields)
...
def print_shot_info(shot_data, latest_version, sg):
    ...
    if shot_data.get('tasks'):
        ...
        for task in shot_data['tasks']:
            ...
            print(f"    Start Date: {task.get('start_date', 'No start date available')}")
            print(f"    Due Date: {task.get('due_date', 'No due date available')}")
