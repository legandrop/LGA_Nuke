...
class SyncApp:
    ...
    def create_widgets(self):
        # Configuracion de estilo y fondo
        self.root.set_appearance_mode("Dark")  # Esto configura el modo oscuro para todos los widgets de ctk
        self.root.set_default_color_theme("blue")  # Puedes elegir el tema de color que prefieras
        
        # Frame de control principal
        control_frame = ctk.CTkFrame(self.root, corner_radius=0)
        control_frame.pack(pady=20, padx=40, fill=ctk.BOTH, expand=True)

        # Frame para los elementos de auto-sync
        autosync_frame = ctk.CTkFrame(control_frame, corner_radius=0)
        autosync_frame.pack(side=ctk.TOP, fill=ctk.X, expand=False)

        # Checkbox para Auto-Sync
        self.auto_sync_check = ctk.CTkCheckBox(autosync_frame, text="Auto-Sync", variable=self.auto_sync, onvalue=True, offvalue=False, command=self.toggle_auto_sync)
        self.auto_sync_check.pack(side=ctk.LEFT, padx=10)

        # Etiqueta "every"
        every_label = ctk.CTkLabel(autosync_frame, text="every")
        every_label.pack(side=ctk.LEFT, padx=5)

        # Dropdown para seleccionar el intervalo
        self.interval_option = ctk.CTkOptionMenu(autosync_frame, values=["1", "15", "30", "60"], command=self.on_interval_change)
        self.interval_option.set("15")  # Valor predeterminado
        self.interval_option.pack(side=ctk.LEFT, padx=5)

        # Etiqueta "minutes"
        minutes_label = ctk.CTkLabel(autosync_frame, text="minutes")
        minutes_label.pack(side=ctk.LEFT, padx=5)

        # Boton Sync Now, ahora debajo y centrado
        self.sync_now_button = ctk.CTkButton(control_frame, text="Sync Now", command=self.sync_now)
        self.sync_now_button.pack(pady=10, padx=10, anchor=ctk.CENTER)  # Ajustado para centrar el boton

        # Etiqueta de estado
        self.status_label = ctk.CTkLabel(self.root, text="Status: Sync Deactivated")
