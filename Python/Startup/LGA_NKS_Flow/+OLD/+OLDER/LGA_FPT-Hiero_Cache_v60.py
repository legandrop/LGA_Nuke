...
class SyncApp:
    def __init__(self, root, shotgrid_manager):
        ...
        config = load_config()
        self.auto_sync_var = tk.IntVar(value=config.get('auto_sync', 0))  # Usar tk.IntVar para manejar el estado
    ...
    def create_widgets(self):
        ...
        # Asegurate de que el checkbox refleje el estado inicial correctamente
    ...
    if os.path.exists(config_path):
        with open(config_path, 'r') as config_file:
            try:
                ...
                # Asegurate de que el archivo no este vacio y tenga la clave necesaria
                if 'auto_sync' in config:
                    return config
            except json.JSONDecodeError:
                pass
    # Retorna configuracion por defecto si no existe el archivo o no se puede cargar
