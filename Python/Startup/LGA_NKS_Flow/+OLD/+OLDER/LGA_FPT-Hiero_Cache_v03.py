...
logging.basicConfig(filename='T:/LGA_Cache.log', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
...
class HieroOperations:
    ...
    def process_all_shots(self, output_file):
        ...
        logging.info("Starting synchronization with SG. Current date and time: {}".format(start_time.strftime("%Y-%m-%d %H:%M:%S")))
            ...
            for key, value in diff.items():
                if 'values_changed' in key:
                    for change in value:
                        ...
                        logging.info(f"Change detected in Shot Index {path}: {change_desc}")
        else:
