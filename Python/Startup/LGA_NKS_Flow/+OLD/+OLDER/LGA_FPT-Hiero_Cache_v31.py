...
class SyncApp:
    ...
    def start_sync(self):
        ...
        # Cambia el estilo para mostrar el estado activo
        style = ttk.Style()
        style.configure('Green.TLabel', foreground='green')  # Define un estilo con texto verde
        self.status_label.config(text="Status: Sync Activated", style='Green.TLabel')
    ...
    def stop_sync(self):
        if self.running:
            ...
            self.running = False
            style = ttk.Style()
            style.configure('Red.TLabel', foreground='red')  # Configura el estilo para texto rojo
            self.status_label.config(text="Status: Sync Deactivated", style='Red.TLabel')
...
def main():
    root = tk.Tk()  # Usamos Tk normalmente
    ...
    # Aplica el tema Sun Valley usando sv_ttk
    sv_ttk.set_theme("dark")  # Opcion para el tema oscuro
