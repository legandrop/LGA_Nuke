...
class ShotGridManager:
    ...
    def __init__(self, json_path):
        ...
        self.task_status_dict = {
            'apr': ('Approved', '#5cb849'),
            'enviad': ('Enviado', '#000000'),
            'wts': ('Waiting to start', '#000000'),
            'corr': ('Corrections', '#2e77d4'),
            'progre': ('In Progress', '#000000'),
            'noread': ('Not Ready To Start', '#000000'),
            'rev': ('Pending Review', '#000000'),
            'pubsh': ('Publish', '#000000'),
            'pbshed': ('Published', '#000000'),
            'ready': ('Ready To Start', '#000000'),
            'rev_di': ('Review Dir', '#c79bb7'),
            'rev_su': ('Review Sup', '#a3557e'),
            'vwd': ('Viewed', '#000000'),
            'check': ('Delivery Checked', '#000000')
        }

    def find_project(self, project_name):
        """Busca un proyecto por nombre en el JSON."""
        return next((p for p in [self.data] if p['project_name'] == project_name), None)
...
class HieroOperations:
    ...
    def add_row_to_table(self, table, gui_table, shot_code, version_number, prev_status, prev_color, new_status, new_color, sg_version_number, sg_status):
        ...
        # Colores por defecto para todas las columnas
        row_colors = ['#8a8a8a'] * 6

        
        # Anadir un espacio al final de cada texto para mejorar la visualizacion        
        shot_item = QTableWidgetItem(shot_code + "   ")
        version_item = QTableWidgetItem(version_number)  
        sg_version_item = QTableWidgetItem(sg_version_number)
        ...
        if int(sg_version_number) > int(version_number):  # Condicion para pintar la columna v_SG
        ...
        # Configuracion de colores para columna de estado previo
        ...
        prev_status_item.setForeground(QBrush(QColor(prev_status_text_color)))
        prev_status_item.setTextAlignment(Qt.AlignCenter)
        
        # Configurar colores para columna de estado nuevo
        ...
        new_status_item.setForeground(QBrush(QColor(new_status_text_color)))
       ...
       # Configuracion de colores que se agregan a la lista para la linea de seleccion
        ...
        if int(sg_version_number) > int(version_number):  # Si la version SG es mayor que la version NKS
    ...
    def change_clip_color(self, item, new_color_hex, task_status, task_name, shot_code):
        ...
        if current_color_hex == new_color_hex:
            return ""
        if (current_color_hex == self.hiero_status_dict['Corrs Lega'] and task_status == 'Review Sup'):
            return ""
    ...
    def get_status_name_by_color(self, color_hex):
        ...
        for status, (name, color) in self.sg_manager.task_status_dict.items():
            if color == color_hex:
                return name
        return "Unknown"
    ...
    def add_custom_tag_to_clip(self, clip, tag_name, tag_description, tag_icon):
        """Anade un tag personalizado a un clip con una descripcion dinamica."""
        new_tag = hiero.core.Tag(tag_name)
        new_tag.setIcon(tag_icon)
        # Asegura que la descripcion sea una cadena
        safe_description = str(tag_description) if tag_description is not None else "New version without notes"
        new_tag.setNote(safe_description)
        clip.addTag(new_tag)
        print(f"Added tag '{tag_name}' with note '{safe_description}' to clip: {clip.name()}")
...
class HieroOperations:
    ...
    def process_selected_clips(self, table, sg_manager):
        """Procesa los clips seleccionados en el timeline de Hiero y muestra los cambios en la tabla."""
        ...
        if seq:
            ...
            print(f"Selected clips count: {len(selected_clips)}")  # Debug: cantidad de clips seleccionados
            if selected_clips:
                ...
                for clip in selected_clips:
                    ...
                    if not file_path or '_comp_' not in os.path.basename(file_path).lower():
                        print("Archivo no valido o no contiene '_comp_'.")
                        continue
                    exr_name = os.path.basename(file_path)
                    base_name, version_number = self.parse_exr_name(exr_name)
                    print(f"EXR Name: {exr_name}, Base Name: {base_name}, Version Number: {version_number}")  # Debug

                    project_name = base_name.split('_')[0]
                    parts = base_name.split('_')
                    shot_code = '_'.join(parts[:5])
                    task_name = parts[5].lower()
                    print(f"Project Name: {project_name}, Shot Code: {shot_code}, Task Name: {task_name}")  # Debug

                    shot = sg_manager.find_shot(project_name, shot_code)
                    if shot:
                        print(f"Found Shot: {shot['shot_name']}")  # Debug
                        task = sg_manager.find_task(shot, task_name)
                        if task:
                            print(f"Found Task: {task['task_type']} with status {task['task_status']}")  # Debug
                            task_status_code = task['task_status']
                            task_status_name, new_color_hex = sg_manager.task_status_dict.get(task_status_code, ("Estado desconocido", "#000000"))
                            current_color_hex = self.get_current_clip_color(clip)
                            print(f"Current Clip Color: {current_color_hex}, New Color: {new_color_hex}")  # Debug

                            change = self.change_clip_color(clip, new_color_hex, task_status_name, task_name, shot_code)
                            print(f"Change Applied: {change}")  # Debug

                            highest_version = sg_manager.find_highest_version_for_shot(shot)
                            sg_version_number = highest_version['version_number'] if highest_version else "No info"
                            sg_status = highest_version['version_status'] if highest_version else "No info"
                            print(f"Highest Version: {sg_version_number}, Status: {sg_status}")  # Debug

                            if change:
                                prev_color_hex = current_color_hex if current_color_hex else "#000000"
                                self.add_row_to_table(table, self.gui_table, shot_code, version_number, current_status, prev_color_hex, task_status_name, new_color_hex, sg_version_number, sg_status)
                                changes_made = True
                        else:
                            print("No se encontraron tareas que coincidan con el nombre para este clip.")
                    else:
                        print(f"No se encontraron datos validos para {shot_code}.")
                project.endUndo()
            else:
                print("No se seleccionaron clips en la linea de tiempo.")
        else:
            print("No se encontro una secuencia activa en Hiero.")

        return changes_made  # Devuelve si se realizaron cambios
...
def main():
    sg_manager = ShotGridManager('T:/LGA_Cache.json')
