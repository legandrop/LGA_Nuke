...
# Agrega la ruta de la subcarpeta 'Data' al sys.path
data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Data')
if data_path not in sys.path:
    sys.path.append(data_path)

# Ahora puedes importar la libreria como de costumbre
...
class ShotGridDownloader:
    ...
    def __init__(self, server_url, script_name, script_key):
        self.cancel_download = False
        self.server_url = server_url
        self.script_name = script_name
        self.script_key = script_key
        self.sg = self.connect_to_shotgrid()

    def connect_to_shotgrid(self):
        try:
            print(f"Conectando a ShotGrid con server_url={self.server_url}, script_name={self.script_name}")
            sg = shotgun_api3.Shotgun(self.server_url, script_name=self.script_name, api_key=self.script_key)
            print("Conexion exitosa a ShotGrid")
            return sg
    ...
    def download_and_save_data(self, project_names, json_SG_output_file):
        ...
        for project_name in project_names:
            if self.cancel_download:
                break
            print(f"Buscando shots en el proyecto '{project_name}'...")

            print("1")
            # Intentar recuperar una lista de proyectos para probar la conexion
            try:
                projects = self.sg.find("Project", [], ['id', 'name'])
            ...
            time_elapsed = end_time - start_time  # Tiempo transcurrido
            print("2")
            ...
            if self.cancel_download:
                break
            print()  # Salto de linea despues de completar la busqueda
        ...
        if not self.cancel_download:
            # Escribir los datos en un archivo JSON solo si no se cancelo
            with open(json_SG_output_file, 'w', encoding='utf-8') as file:
                json.dump(output_data, file, ensure_ascii=False, indent=4)

        print("Proceso completado. JSON generado con exito." if not self.cancel_download else "Proceso cancelado.")
...
class HieroOperations:
    ...
    def __init__(self, root, shotgrid_manager):
        ...
        ctk.set_default_color_theme(resource_path("LGA_FPT-Hiero_Cache_CCTK_Theme.json"))
    ...
    def log_version_details(self, shot_index, task_index, version, versions_uploaded, project_data, added=True):
        ...
        logging.info(f"Logging version {action}: {version['version_number']} in shot index {shot_index}, task index {task_index}")
        if shot_index != 0 and task_index != 0:
            ...
            logging.info(f"Version {action} in shot {shot_name}, task {task_type}: {version['version_number']}")
        if added:
            versions_uploaded.append(version['version_number'])
        logging.info(f"Version {action}: {version['version_number']}")
        logging.info(f"Version description: {version.get('version_description', 'No description provided')}")
    ...
    def log_version_changes(self, item_key, new_value, project_data, versions_uploaded):
        ...
        shot_index = item_key.split("root['shots'][")[1].split(']')[0]
...
class SyncApp:
    def __init__(self, root, server_url, script_name, script_key):
        self.root = root
        self.server_url = server_url
        self.script_name = script_name
        self.script_key = script_key
    ...
    def create_widgets(self):
        ...
        version_label = ctk.CTkLabel(bottom_frame, text="v1.1", anchor="se")
    ...
    def toggle_sync(self):
        if self.download_thread and self.download_thread.is_alive():
            ...
            if not self.sg_downloader:
                self.sg_downloader = ShotGridDownloader(self.server_url, self.script_name, self.script_key)
    ...
    def download_and_sync(self):
        ...
        # Reutilizar la instancia de ShotGridDownloader existente
        if not self.sg_downloader:
            self.sg_downloader = ShotGridDownloader(self.server_url, self.script_name, self.script_key)
        json_SG_output_file = resource_path('LGA_FPT-Hiero_Cache_SG.json')
        print("001")
        # Crear y comenzar el hilo de descarga, reutilizando la misma instancia de sg_downloader
        self.download_thread = threading.Thread(target=self.download_project_data_in_thread, args=(self.project_names, json_SG_output_file))
        self.download_thread.start()
        print("002")
        ...
        self.stop_indeterminate_progress()  # Detener la barra de progreso en modo indeterminado
        
        # Reactivar la tecla ESC despues de la sincronizacion
        self.root.bind("<Escape>", self.on_closing)        

    def download_project_data_in_thread(self, project_names, json_SG_output_file):
        if self.sg_downloader:
            self.sg_downloader.download_and_save_data(project_names, json_SG_output_file)
    ...
    def open_log_file(self):
        log_file_path = os.path.join(self.log_path_var.get(), 'LGA_FPT-Hiero_Cache.log')
        if os.path.exists(log_file_path):
            os.startfile(log_file_path)  # Esto abrira el archivo con la aplicacion predeterminada del sistema operativo

    def clear_log_file(self):
        log_file_path = os.path.join(self.log_path_var.get(), 'LGA_FPT-Hiero_Cache.log')
        with open(log_file_path, 'w', encoding='utf-8') as file:
            file.write("")  # Borra el contenido del archivo
    ...
    def initialize_sg_manager(self):
        json_file_path = resource_path('LGA_FPT-Hiero_Cache_SG.json')
    ...
    def sync_now(self):
        # Ejecutar el proceso de sync en un hilo separado (tanto por el boton como por la cuenta regresiva de autosync)
    ...
    def job(self):
        ...
        output_file = os.path.join(resource_path(''), 'LGA_FPT-Hiero_Cache_Local.json')
...
def resource_path(relative_path):
    ...
    relative_path_with_files = os.path.join("Data", relative_path)

    return os.path.join(base_path, relative_path_with_files)
...
def main():
    # Datos de conexion
    server_url = 'https://wanka.shotgrid.autodesk.com'
    script_name = 'Support'
    script_key = '_csauvoliqbmpkvvKaOsgyhoy'
    ...
    app = SyncApp(root, server_url, script_name, script_key)
    root.mainloop()
