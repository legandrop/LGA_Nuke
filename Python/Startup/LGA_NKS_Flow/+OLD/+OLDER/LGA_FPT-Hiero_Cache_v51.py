...
class SyncApp:
    ...
    def create_tray_icon(self):
        '''Inicializa el icono del system tray con las opciones de menu correspondientes.'''
        icon = resource_path("LGA.ico") 

        hover_text = "SyncApp System Tray"
        menu_options = (('Abrir SyncApp', None, self.show_window),
                        ('Sincronizar ahora', None, self.sync_now),
                        ('Cerrar SyncApp', None, self.on_closing))
        self.tray_icon = SysTrayIcon.SysTrayIcon(icon, hover_text, menu_options, on_quit=self.on_close)
    ...
    def hide_window(self, *args):
        '''Oculta la ventana principal de la aplicacion, no la cierra.'''
        self.root.withdraw()

    def on_close(self, *args):
        '''Limpieza antes de cerrar completamente.'''
        if self.tray_icon:
            self.tray_icon.shutdown()  # Metodo para cerrar correctamente el icono del tray
        self.root.destroy()
        sys.exit()
