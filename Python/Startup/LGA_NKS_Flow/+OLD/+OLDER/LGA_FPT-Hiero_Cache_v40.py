...
class SyncApp:
    ...
    def create_widgets(self):
        ...
        autosync_frame = ctk.CTkFrame(control_frame)
        autosync_frame.pack(side=ctk.LEFT, padx=10, fill=ctk.X, expand=True)

        # Checkbox para Auto-Sync
        self.auto_sync_check = ctk.CTkCheckBox(autosync_frame, text="Auto-Sync", command=self.toggle_auto_sync)
        ...
        # Frame para el boton Sync Now
        sync_frame = ctk.CTkFrame(control_frame)
        sync_frame.pack(pady=20, padx=40, fill=ctk.X)
        
        # Boton Sync Now, centrado
        self.sync_now_button = ctk.CTkButton(sync_frame, text="Sync Now", command=self.sync_now)
        self.sync_now_button.pack(side=ctk.LEFT, padx=10, anchor=ctk.CENTER)
