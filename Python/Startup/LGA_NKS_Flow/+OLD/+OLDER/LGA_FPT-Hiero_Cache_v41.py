...
class SyncApp:
    ...
    def create_widgets(self):
        ...
        self.auto_sync_check.pack(side=ctk.LEFT, padx=10, fill=ctk.X, expand=True)

        # Etiqueta "every"
        every_label = ctk.CTkLabel(control_frame, text="every")
        every_label.pack(side=ctk.LEFT, padx=5, fill=ctk.X, expand=True)
        ...
        self.interval_option.pack(side=ctk.LEFT, padx=5, fill=ctk.X, expand=True)

        # Etiqueta "minutes"
        minutes_label = ctk.CTkLabel(control_frame, text="minutes")
        minutes_label.pack(side=ctk.LEFT, padx=5, fill=ctk.X, expand=True)
        ...
        # Boton Sync Now, directamente en la ventana raiz, entre status y next sync
        self.sync_now_button = ctk.CTkButton(self.root, text="Sync Now", command=self.sync_now)
        self.sync_now_button.pack(pady=10)
    ...
    def adjust_window_size(self):
        ...
        width = self.root.winfo_reqwidth() + 10  # Ancho requerido mas 200 pixeles
        height = self.root.winfo_reqheight() + 10  # Altura requerida mas 50 pixeles
