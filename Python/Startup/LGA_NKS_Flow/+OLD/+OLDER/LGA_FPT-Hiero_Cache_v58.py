...
class SyncApp:
    def __init__(self, root, shotgrid_manager):
        ...
        self.root.bind("<Unmap>", self.withdraw_window)  # Detecta cuando la ventana se minimiza
    ...
    def withdraw_window(self, event=None):
        print("Withdraw called: subiendo =", self.subiendo, "contador =", self.contador)
        if self.contador == 0 and not self.subiendo:
            self.subiendo = True
            print("No minimiza, cambia subiendo a True")
        else:
            if self.contador == 4 and self.subiendo:
                self.subiendo = False
                #self.contador = 0
                print("No minimiza, reinicia contador y cambia subiendo a False")
            else:
                if self.subiendo:
                    self.contador += 1
                    print("Subiendo, contador aumenta:", self.contador)
                if not self.subiendo:
                    self.contador -= 1
                    print("Bajando, contador disminuye:", self.contador)
                if self.contador in [1, 2, 3] and self.subiendo:
