...
class SyncApp:
    ...
    def setup_system_tray(self):
        icon_image = Image.open(resource_path("LGA.ico"))  # Asegurate de tener un icono adecuado
        
        # Configura el menu con una accion por defecto para clic izquierdo
        menu = Menu(item('Abrir', self.show_window, default=True),
                    item('Salir', self.quit_window))
        
        # Crear el icono del tray con el menu configurado
        self.tray_icon = pystray.Icon("SyncApp", icon_image, "SyncApp", menu)
        self.tray_icon.run()

    def show_window(self, icon, item):
        self.root.deiconify()
        # Considerar si quieres detener el icono del tray aqui o no
        self.tray_icon.stop()
