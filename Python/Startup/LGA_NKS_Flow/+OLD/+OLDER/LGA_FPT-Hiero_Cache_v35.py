...
def create_image():
    # Crear una imagen simple para el icono de la bandeja del sistema
    image = Image.new('RGB', (64, 64), color=(53, 53, 53))  # Color de fondo oscuro
    return image
...
def minimize_to_tray(window):
    window.withdraw()
    image = create_image()
    icon = TrayIcon("App Icon", image, "My Application", menu=menu(item('Restore', lambda icon: show_window(icon, window))))
    icon.run()

class CustomToplevel(tk.Toplevel):
    def __init__(self, master=None, **kwargs):
        super().__init__(master, **kwargs)
        self.overrideredirect(True)  # Ocultar el titlebar predeterminado
        self.setup_ui()

    def setup_ui(self):
        self.titlebar = tk.Frame(self, bg='#333333', relief='raised', bd=2)
        self.titlebar.pack(fill=tk.X)

        self.close_button = ttk.Button(self.titlebar, text='X', command=self.on_closing)
        self.close_button.pack(side=tk.RIGHT, padx=5, pady=5)

        self.minimize_button = ttk.Button(self.titlebar, text='-', command=lambda: minimize_to_tray(self))
        self.minimize_button.pack(side=tk.RIGHT, padx=5, pady=5)

        self.title_label = ttk.Label(self.titlebar, text='My Custom Window', background='#333333', foreground='white', font=('Arial', 12))
    ...
    def on_closing(self):
        ...
        print("cerrando 2")
        self.root.quit()  # Cierra el bucle principal
        print("cerrando 3")
        self.root.destroy()  # Destruye la ventana root
...
class HieroOperations:
    ...
    def __init__(self, root, shotgrid_manager):
        ...
        self.root = tk.Toplevel(root)  # Cambiado a Toplevel
    ...
    def show_initial_dialog(self, versions):
        ...
        sv_ttk.set_theme("dark")  # Asegura que el tema de Sun Valley se aplique en esta ventana tambien
        center_window(dialog, 400, 300)
        dialog.attributes('-topmost', True)  # Asegura que la ventana se mantenga en el frente

        font_family = "Segoe UI"
        font_size = 10
        message_frame = ttk.Frame(dialog, padding=(20, 20))
        message_frame.pack(fill=tk.BOTH, expand=True)

        count = len(versions)
        message = f"{count} new versions pulled from FPT"
        ttk.Label(message_frame, text=message, font=(font_family, font_size)).pack()

        button_frame = ttk.Frame(dialog, padding=(0, 10))
        button_frame.pack(fill=tk.X)

        # Almacena el ID del evento after para poder cancelarlo mas tarde
        self.after_id = None

        ok_button = ttk.Button(button_frame, text="OK (30)", command=lambda: self.close_dialog_with_countdown(dialog),
                               style='TButton')
        ok_button.pack(side=tk.RIGHT, padx=10)

        details_button = ttk.Button(button_frame, text="Details",
...
class SyncApp(CustomToplevel):
    def __init__(self, master, root, shotgrid_manager):
        super().__init__(master)  # Llama al constructor de CustomToplevel
        self.root = root  # Guarda la referencia al root
        self.set_title("ShotGrid Sync Manager")  # Configura el titulo usando el metodo de CustomToplevel
        self.withdraw()  # Oculta la ventana inmediatamente despues de crearla para evitar parpadeos


        self.sg_manager = shotgrid_manager
        self.hiero_ops = HieroOperations(self, self.sg_manager)  # Cambiar root a self, ya que ahora SyncApp es una ventana
        self.auto_sync = tk.BooleanVar(value=False)  # Define y establece la variable

        self.create_widgets()
        self.adjust_window_size()  # Ajusta el tamano y posicion de la ventana

        self.scheduler_thread = None
        self.running = False
        self.protocol("WM_DELETE_WINDOW", self.on_closing)  # Usar self en lugar de self.root

        self.deiconify()  # Muestra la ventana despues de configurarla

    def create_widgets(self):
        font_family = "Segoe UI"
        font_size = 10
        style = ttk.Style()
        style.configure('TCheckbutton', background='#333333', foreground='#E0E0E0', font=(font_family, font_size))
        style.configure('TButton', background='#333333', foreground='#E0E0E0', font=(font_family, font_size))
        style.configure('TLabel', background='#333333', foreground='red', font=(font_family, font_size))
        style.configure('Green.TLabel', foreground='green')  # Un estilo adicional para el texto verde

        control_frame = ttk.Frame(self, style='TFrame')  # Utiliza ttk.Frame
        ...
        self.auto_sync_check = ttk.Checkbutton(control_frame, text="Auto-Sync", variable=self.auto_sync,
                                               command=self.toggle_auto_sync, style='TCheckbutton')
        ...
        self.interval_dropdown = ttk.OptionMenu(control_frame, self.interval_option, "1", "15", style='TMenubutton')
        self.interval_dropdown.pack(side=tk.LEFT, padx=5)

        # Boton Sync Now
        self.sync_now_button = ttk.Button(control_frame, text="Sync Now", command=self.sync_now, style='TButton')
        self.sync_now_button.pack(side=tk.LEFT, padx=10)

        # Etiqueta de estado
        self.status_label = ttk.Label(self, text="Status: Sync Deactivated", style='TLabel')
        self.status_label.pack(pady=10)

        # Etiqueta para la cuenta regresiva
        self.countdown_label = ttk.Label(self, text="Next Sync: -- sec", style='TLabel')
    ...
    def update_countdown(self):
        if self.running:
            ...
            if remaining_seconds > 0:
                ...
                self.after(1000, self.update_countdown)  # Programa la proxima actualizacion en un segundo
                ...
                self.sync_after_id = self.after(1000, self.sync_now)  # Espera un segundo antes de ejecutar la sincronizacion
        else:
    ...
    def adjust_window_size(self):
        self.update()  # Importante: actualiza el estado de la ventana
        width = self.winfo_reqwidth() + 200  # Ancho requerido mas 200 pixeles
        height = self.winfo_reqheight() + 50  # Altura requerida mas 50 pixeles
        self.geometry(f"{width}x{height}")  # Establece la nueva geometria
...
def main():
    ...
    app = SyncApp(root, root, sg_manager)
    root.mainloop()
