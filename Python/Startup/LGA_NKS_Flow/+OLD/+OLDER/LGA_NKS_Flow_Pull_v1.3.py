...
____________________________________________________________________________
  ...
  LGA_NKS_Flow_Pull v1.3 - 2024 - Lega Pugliese
...
class ShotGridManager:
    ...
    def __init__(self, json_path):
        ...
        self.task_status_dict = {
            ...
            'progre': ('In Progress', '#7d4cff'),
            'noread': ('Not Ready To Start', '#000000'),
            'rev': ('Pending Review', '#000000'),
            'pubsh': ('Publish', '#244c19'),
            'pbshed': ('Published', '#244c19'),
            'ready': ('Ready To Start', '#f9da00'),
            'rev_di': ('Review Dir', '#c79bb7'),
            'revleg': ('Review Lega', '#69135e'),
            'rev_su': ('Review Sup', '#a3557e'),
            'vwd': ('Viewed', '#000000'),
            'check': ('Delivery Checked', '#244c19')
        }
            ...
            if selected_clips:
                ...
                for clip in selected_clips:
                    if isinstance(clip, hiero.core.EffectTrackItem):  # Verificar si es un efecto
                        #print(f"Ignore effect item: {clip.name()}")
                        pass
                    else:                
                            ...
                            if task:
                                ...
                                if change or sg_version_number > version_number:
                                    ...
                                    if sg_version_number > version_number:
                                        self.add_custom_tag_to_clip(clip, "Updated Version", sg_description, "icons:TagYellow.png")

                            else:
                                #print("No matching tasks found for this clip.")
                                pass
                        else:
                            #print(f"No valid data found for {shot_code}.")
                            pass
                #project.endUndo()
            else:
...
def FPT_Hiero():
    ...
    json_path = os.path.join(script_path, 'Data', 'LGA_FPT-Hiero_Cache_Local.json')
