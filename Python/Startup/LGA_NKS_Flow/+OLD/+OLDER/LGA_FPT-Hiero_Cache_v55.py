...
class SyncApp:
    ...
    def setup_system_tray(self):
        ...
        # Configura el menu con una accion por defecto para clic izquierdo
        self.menu = Menu(item('Abrir', self.show_window, default=True),
                         item('Salir', self.quit_window))
        # Crear el icono del tray con el menu configurado
        self.tray_icon = pystray.Icon("SyncApp", self.icon_image, "SyncApp", self.menu)
        self.tray_icon.run()

    def recreate_tray_icon(self):
        # Verifica si hay una instancia anterior del icono y detenla si es necesario
        if hasattr(self, 'tray_icon'):
            self.tray_icon.stop()
        # Crea una nueva instancia del icono y lo inicia.
        self.tray_icon = pystray.Icon("SyncApp", self.icon_image, "SyncApp", self.menu)
        self.tray_icon.run()
    ...
    def on_minimize(self, event=None):
        print("Minimizing")
        self.root.withdraw()
        self.recreate_tray_icon()  # Crea e inicia una nueva instancia del icono cada vez que se minimiza
    ...
    def quit_window(self, icon, item):
        ...
        if hasattr(self, 'tray_icon'):
            print("if si")
            self.tray_icon.stop()  # Stops the system tray icon
        self.root.quit()  # Quits the mainloop
        self.root.destroy()  # Destroys the root window, ensuring complete closure
