...
class HieroOperations:
    ...
    def __init__(self, root, shotgrid_manager):
        ...
        # Cambiar color de la barra de titulo
        hwnd = self.root.winfo_id()
        change_title_bar_color(hwnd, '303030')

        self.root.deiconify()  # Muestra la ventana despues de configurar el tema
...
class SyncApp:
    def __init__(self, root, shotgrid_manager):
        ...
        self.sg_manager = shotgrid_manager
        self.hiero_ops = HieroOperations(root, self.sg_manager)  # Asegurate de que HieroOperations no muestra su ventana innecesariamente

        self.auto_sync = tk.BooleanVar(value=False)  # Define la variable antes de crear los widgets
        ...
        # Muestra la ventana solo despues de que todo esta configurado y centrado
        self.root.deiconify()

        # Cambiar color de la barra de titulo despues de que la ventana sea visible
        hwnd = self.root.winfo_id()
        change_title_bar_color(hwnd, '303030')
    ...
    def create_widgets(self):
        ...
        control_frame = ttk.Frame(self.root, style='TFrame')  # Utiliza ttk.Frame
        ...
        # Etiqueta de estado
        self.status_label = ttk.Label(self.root, text="Status: Sync Deactivated", style='TLabel')
        self.status_label.pack(pady=10)

        # Etiqueta para la cuenta regresiva
        self.countdown_label = ttk.Label(self.root, text="Next Sync: -- sec", style='TLabel')
...
# Funcion para cambiar el color de la barra de titulo
def change_title_bar_color(hwnd, color):
    # Convertir color hexadecimal a entero
    color = int(color, 16) + 0xFF000000  # Anadir alpha si es necesario
    # Usar ctypes para cambiar el color de la barra
    windll.dwmapi.DwmSetWindowAttribute(hwnd, 20, ctypes.byref(ctypes.c_int(color)), 4)
