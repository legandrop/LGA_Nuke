...
class SyncApp:
    ...
    def settings_window(self):
       ### Ventana que muestra cuantas versiones nuevas hay
        settingsW = ctk.CTkToplevel(self.root)
        settingsW.title(" ")
        ...
        # Establecer el icono, asegurandose de que el camino al archivo sea correcto
        icon_path = resource_path("no_icon.ico")
        settingsW.iconbitmap(default=icon_path)

        # Solucion para el problema en Windows con customtkinter
        if platform.startswith("win"):
            settingsW.after(200, lambda: settingsW.iconbitmap(icon_path))  # Reestablecer el icono en Windows        
        
        set_title_bar_color(settingsW, BG_Color, TXT_Color)  # Set title bar color

        # Configura el resto de los elementos de la ventana
        settingsW.attributes('-topmost', True)  # Asegura que la ventana se mantenga en el frente

        
        # Se crea un marco sin padding y luego se configura el padding interno
        settings_frame = ctk.CTkFrame(settingsW)
        ...
        ok_button = ctk.CTkButton(button_frame, text="OK", width=20, command=lambda: self.update_log_path(log_path_entry.get(), settingsW))
        ok_button.pack(side=ctk.RIGHT, padx=(0,20), pady=5)

        # Boton Cancel para cerrar la ventana
        cancel_button = ctk.CTkButton(button_frame, text="Cancel", command=settingsW.destroy, width=20)
        ...
        browse_button.pack(side=ctk.RIGHT, padx=10, pady=5)        

        # Almacena el ID del evento after para poder cancelarlo mas tarde
        self.after_id = None

        #ok_button = ctk.CTkButton(button_frame, text="OK", command=settingsW.destroy, width=30)
        #ok_button.pack(side=ctk.RIGHT, padx=10, pady=(10, 30))

        self.adjust_window_size(settingsW)
        
        settingsW.deiconify() # Muestra la ventana solo despues de que todo esta configurado y centrado
        
        settingsW.focus_force() # Asegura que la ventana de configuraciones este enfocada y en el frente

        # Vincular la tecla Esc para cerrar la ventana
        settingsW.bind("<Escape>", lambda event: settingsW.destroy())    
    ...
    def update_log_path(self, new_path, window):
        ...
        for handler in logging.root.handlers[:]:  # Elimina todos los handlers antiguos
...
def main():
    ...
    logging.basicConfig(filename=log_file_path, level=logging.INFO, format='%(message)s')
    #logging.basicConfig(filename='T:/LGA_FPT-Hiero_Cache.log', level=logging.INFO, format='%(message)s')
