#pyinstaller --onefile --add-data "Data/no_icon.ico;Data" --add-data "Data/LGA.ico;Data" --add-data "Data/settings_off.png;Data" --add-data "Data/settings_on.png;Data" --add-data "Data/CTkScrollableDropdown/ctk_scrollable_dropdown.py;Data/CTkScrollableDropdown" --add-data "Data/LGA_FPT-Hiero_Cache_CCTK_Theme.json;Data" -i "Data/LGA.ico" LGA_FPT-Hiero_Cache.py
...
class HieroOperations:
    ...
    def handle_differences(self, diff, project_data, versions_uploaded):
        ...
        for key, value in diff.items():
            ...
            # Imprimir el estado actual de versions_uploaded
            logging.info("Current state of versions_uploaded:")
            for version in versions_uploaded:
    ...
    def log_version_changes(self, item_key, new_value, project_data, versions_uploaded):
        ...
        logging.info(f"Processing item_key: {item_key}")

        # Verificar si el formato es el esperado
        if "root['shots'][" in item_key:
            try:
                shot_index = item_key.split("root['shots'][")[1].split(']')[0]
            except IndexError:
                logging.error(f"Formato inesperado en item_key: {item_key}")
                return
    ...
    def log_added_or_removed_items(self, change_type, item_key, item_value, project_data, versions_uploaded):
        ...
        if isinstance(new_value, dict) and 'version_number' in new_value:
            ...
            logging.info("____________________________\n")
        elif isinstance(new_value, dict) and 'versions' in new_value:
            # Manejar el caso de un task con varias versiones
            for version in new_value['versions']:
                versions_uploaded.append(version['version_number'])
                logging.info(f"Version added/removed: {version['version_number']}")
                logging.info(f"Version description: {version.get('version_description', 'No description provided')}")
                logging.info(f"Version status: {version['version_status']}")
                logging.info(f"Version date: {version['version_date']}")
                logging.info("____________________________\n")
            ...
            # Agregar detalles adicionales del item_value
            logging.error(f"Detalles adicionales del formato inesperado: {item_value}")
            logging.info("____________________________\n")
...
class SyncApp:
    ...
    def download_and_sync(self):
        ...
        self.sg_downloader = ShotGridDownloader(self.url, self.script_name, self.api_key)
        json_SG_output_file = os.path.join(Script_Path, 'LGA_FPT-Hiero_Cache_SG.json')
    ...
    def initialize_sg_manager(self):
        json_file_path = os.path.join(Script_Path, 'LGA_FPT-Hiero_Cache_SG.json')
    ...
    def job(self):
        ...
        output_file = os.path.join(Script_Path, 'LGA_FPT-Hiero_Cache_Local.json')
...
def main():
    ...
    script_name = "Support"
    api_key = "_csauvoliqbmpkvvKa0sgyhoy"
    ...
    app = SyncApp(root, url, script_name, api_key)
    root.mainloop()
