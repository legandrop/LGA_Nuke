...
class SyncApp:
    ...
    def create_widgets(self):
        ...
        ######################
        my_image = ctk.CTkImage(dark_image=Image.open(resource_path("settings_off.png")), size=(30, 30))
        #my_image = PhotoImage(master = bottom_frame, width = 20, height = 20, file=(resource_path("settings_off.png")))

        # Crear y configurar el boton con la imagen
        self.image_button = ctk.CTkButton(
            master=bottom_frame,
            image=my_image,
            text=" ",
            #width=20,  # Ajusta el ancho segun la necesidad
            #height=20,  # Ajusta la altura segun la necesidad
            command=self.button_function,  # Funcion a ejecutar cuando se presione el boton
            compound='left'
        )
        self.image_button.pack(side=ctk.LEFT, padx=30)     

        #image_label = ctk.CTkLabel(master=self.root, image=my_image, text="")
        
        ##########
        """
        # Cargar la imagen y convertirla a CTkImage
        settings_image = PhotoImage(master = bottom_frame, width = 20, height = 20, file=(resource_path("settings_off.png")))
        #settings_ctk_image = ctk.CTkImage(image=settings_image)  # Rectificacion aqui
        ...
        self.settings_button.pack(side=ctk.LEFT, padx=30)

        # Asegurar que la imagen se mantiene en memoria
