...
class HieroOperations:
    ...
    def process_all_shots(self, output_file):
        ...
        logging.info("____________________________+++++++++++++++____________________________\n")
        ...
        config = load_config()
        logging.info(f"++++++Configuracion cargada: {config}")  # Agregar este print
        project_names = config.get('project_names', ['DefaultProjectName'])  # Usar un valor predeterminado si no se encuentra

        # Inicializa la estructura de proyectos al comienzo del proceso total
        all_projects_data = {'projects': []}
        all_versions_uploaded = []  # Lista para todas las versiones nuevas de todos los proyectos        
        ...
        for project_name in project_names:
            logging.info(f"++++++Buscando shots para el proyecto: {project_name}")  # Agregar este print
            shots = self.sg_manager.find_project_shots([project_name])
            logging.info(f"++++++Shots encontrados: {shots}")  # Agregar este print
            project_data = {
            ...
            all_projects_data['projects'].append(project_data)  # Acumula los datos de cada proyecto
            ...
            with open(output_file, 'w', encoding='utf-8') as file:
                logging.info(f"++++++Datos que seran escritos en el JSON: {project_data}")  # Agregar este print para depuracion
    ...
    def handle_differences(self, diff, project_data, versions_uploaded):
        ...
        logging.info(f"++++++Procesando diferencias: {diff}")  # Imprime la diferencia completa para ver que se esta detectando
    ...
    def process_change(self, item, change_type):
        ...
        # Aqui puedes anadir la logica para manejar cambios especificos, como agregar o eliminar elementos
...
def main():
    ...
    # Cargar la ultima version existente del JSON de SG
    ...
    sg_manager = ShotGridManager(json_file_path)
