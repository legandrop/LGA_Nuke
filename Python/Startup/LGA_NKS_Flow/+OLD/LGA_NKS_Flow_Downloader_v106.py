...
class ShotGridDownloader:
    ...
    def find_tasks_for_shot(self, shot_id):
        ...
        fields = ['id', 'content', 'description', 'sg_status_list', 'task_assignees']  # Anadir task_assignees
        tasks = self.sg.find("Task", filters, fields)
        for task in tasks:
            if task['task_assignees']:
                task['assigned_to'] = task['task_assignees'][0]['name']  # Asignar el primer asignado
            else:
                ...
                for task in tasks:
                    if 'Comp' in task['content']:
                        ...
                        task_data = {
                            ...
                            "task_assigned_to": task.get('assigned_to', 'No asignado'),  # Nuevo campo
                            "versions": []
                        }
        ...
        for task in shot['tasks']:
            if 'Comp' in task['task_type']:
                task_data = {
                    ...
                    "task_assigned_to": task.get('task_assigned_to', 'No asignado'),  # Anadir este campo
                    "versions": []
                }
                for version in task['versions']:
                    version_data = {
...
class SyncApp:
    ...
    def create_widgets(self):
        ...
        version_label = ctk.CTkLabel(bottom_frame, text="v1.2", anchor="se")
