...
class HieroOperations:
    ...
    def new_versions_window(self, versions):
        ...
        if platform.system() == "Windows":
            dialog.after(200, lambda: setup_window_icon(dialog, icon_path))

        set_title_bar_color(dialog, BG_Color, TXT_Color)
        ...
        details_button = ctk.CTkButton(button_frame, text="Details", command=lambda: [dialog.destroy(), self.uploaded_versions_window(versions)], width=60)
        details_button.pack(side=ctk.RIGHT, padx=10, pady=(10, 30))

        def countdown(count=30):
            if dialog.winfo_exists():
                ok_button.configure(text=f"OK ({count})")
                if count > 0:
    ...
    def uploaded_versions_window(self, versions):
        ...
        set_title_bar_color(details_window, BG_Color, TXT_Color)
        if platform.system() == "Windows":
            details_window.after(200, lambda: setup_window_icon(details_window, icon_path))
        ...
        initial_height = details_window.winfo_screenheight() * 0.9
...
class SyncApp:
    def __init__(self, root, url, script_name, api_key):
        logging.info("Inicializando SyncApp...")
        ...
        self.root.title("LGA_NKS_Flow_Downloader")
        self.tooltip_window = None
        self.running = False
        self.progress_length = 0.35
        ...
        self.log_file_path = config.get('log_file_path', Script_Path)
        self.project_names = config.get('project_names', ["", "", "", ""])
        self.project_vars = [tk.StringVar(value=name) for name in self.project_names]

        # Log inicializacion de variables clave
        logging.info(f"start_minimized: {self.start_minimized}, close_to_tray: {self.close_to_tray}")

        # Intenta comentar temporalmente la configuracion de la barra de titulo y los iconos
        # set_title_bar_color(self.root, BG_Color, TXT_Color)
        # icon_path = resource_path("no_icon.ico")
        # self.root.iconbitmap(default=icon_path)

        # Si esta desactivado, no ocultes la ventana al inicio
        if not self.start_minimized:
            self.root.deiconify()

        # Comentar temporalmente la logica de bandeja
        ...
        self.create_widgets()
        self.adjust_window_size(self.root)
        ...
        logging.info("SyncApp inicializada correctamente.")
    ...
    def create_widgets(self):
        ...
        if platform.system() == "Windows":
            # Ajuste especifico para Windows
            config = load_config()
            current_interval = config.get('sync_interval', '15')
            self.interval_option = tk.StringVar(value=current_interval)
    ...
    def show_window(self, icon=None, item=None):
        if platform.system() == "Windows":
            ...
            # Comportamiento multiplataforma/macos
            self.root.after(0, self.root.deiconify)
            if icon:
                icon.stop()
...
def center_window(window, width=400, height=200):
    ...
    y = int((screen_height / 2) - (height / 2) - 40)  # Restar 40 pixeles a la coordenada 'y' para compensar la barra de tareas
...
def main():
    try:
        ...
        logging.info("Iniciando la aplicacion con tkinter...")

        root = tk.Tk()
        root.title("Prueba con tkinter")
        root.geometry("400x300")

        label = tk.Label(root, text="Ventana de Prueba con Tkinter")
        label.pack(pady=20)

        logging.info("Ejecutando mainloop...")
        root.mainloop()

    except Exception as e:
        logging.error(f"Error durante la ejecucion: {e}", exc_info=True)
