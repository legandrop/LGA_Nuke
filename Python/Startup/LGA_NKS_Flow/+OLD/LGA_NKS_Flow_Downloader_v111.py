...
class SyncApp:
    ...
    def initialize_sg_manager(self):
        json_SG_output_file = os.path.join(Script_Path, 'LGA_NKS_Flow_Downloader_SG.json')
        if os.path.exists(json_SG_output_file):
            self.sg_manager = ShotGridManager(json_SG_output_file)  # Usar ShotGridManager en lugar de ShotGridDownloader
            self.hiero_ops = HieroOperations(self.root, self.sg_manager)  # Pasar sg_manager a HieroOperations
        else:
            logging.error("No se encontro el archivo de salida JSON para inicializar ShotGridManager.")

    def save_projects(self):
        # Actualizar project_names basado en los valores actuales de project_vars
        self.project_names = [var.get() for var in self.project_vars]

        # Actualizar configuracion
        config = load_config()
        config['project_names'] = self.project_names  # Almacenar los nombres de los proyectos
        save_config(config)

        # Actualizar la instancia de HieroOperations si esta definida
        if self.hiero_ops:
            self.hiero_ops.sg_manager.data = self.project_names

        self.settingsW.destroy()  # Cerrar la ventana de configuracion

    def add_project_entry(self):
        # Agregar un nuevo StringVar para el nuevo proyecto
        new_var = tk.StringVar()
        self.project_vars.append(new_var)
        
        # Llamar a refresh_project_entries para actualizar los widgets
        self.refresh_project_entries()
    ...
    def refresh_project_entries(self):
        ...
        for var in self.project_vars:
            entry = ctk.CTkEntry(self.project_entries_frame, textvariable=var)
            entry.pack(fill=ctk.X, expand=True, padx=2, pady=2)
    ...
    def on_interval_change(self, selected_value):
        # Actualizar el valor de sync_interval en la configuracion
        config = load_config()
        config['sync_interval'] = selected_value
        save_config(config)
    ...
    def start_sync(self):
        # Programar la primera sincronizacion inmediata y luego en intervalos
        self.sync_now()
        self.update_countdown()
    ...
    def browse_log_path(self, entry):
        # Abrir el cuadro de dialogo de seleccion de archivo y actualizar el entry con la ruta seleccionada
        file_path = filedialog.asksaveasfilename(defaultextension=".log", filetypes=[("Log files", "*.log"), ("All files", "*.*")])
        if file_path:
            entry.delete(0, ctk.END)
            entry.insert(0, file_path)

    def update_log_path(self, new_log_path, settingsW):
        # Actualizar el archivo de configuracion con la nueva ruta del archivo de log
        self.log_file_path = new_log_path
        config = load_config()
        config['log_file_path'] = new_log_path
        save_config(config)
        settingsW.destroy()  # Cerrar la ventana de configuracion
    ...
    def withdraw_window(self):
        self.root.withdraw()
        if platform.system() == "Darwin":  # Verifica si es macOS
            self.icon.stop()  # Detener el icono de pystray en macOS
        self.icon = pystray.Icon("name", self.image, "title", self.menu)
        self.run_tray_icon()
    ...
    def setup_tooltip(self, widget, text):
        # Mostrar tooltip al pasar el mouse sobre el widget
        widget.bind("<Enter>", lambda event: self.show_tooltip(event, text))
        widget.bind("<Leave>", self.hide_tooltip)

    def show_tooltip(self, event, text):
        # Crear y mostrar la ventana del tooltip
    ...
    def hide_tooltip(self, event):
        # Ocultar la ventana del tooltip
...
def set_title_bar_color(window, title_bar_color, title_text_color):
    if platform.system() == "Windows" and platform.release() == "10":
        ...
        # Cambiar el color del texto y el background de la barra de titulo
        DWMWA_USE_IMMERSIVE_DARK_MODE = 20
        try:
            hwnd = windll.user32.GetParent(window.winfo_id())
            set_window_attribute = windll.dwmapi.DwmSetWindowAttribute
            set_window_attribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE, byref(c_int(1)), sizeof(c_int))
            window.wm_title(window.wm_title())  # Forzar una actualizacion de la barra de titulo
        except Exception as e:
            print(f"Error setting dark mode attribute: {e}")

    # Ajustar el color del background y el texto de la barra de titulo
    window.wm_title(window.wm_title())  # Forzar una actualizacion de la barra de titulo
...
def center_window(window, width, height):
    # Ajustar tamano y centrar la ventana
