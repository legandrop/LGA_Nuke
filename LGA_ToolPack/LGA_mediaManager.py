...
class SettingsWindow(QWidget):
    def __init__(self, settings_data, parent=None):
        ...
        self.settings_data = settings_data.copy()  # Hacemos una copia en memoria del diccionario de configuracion
        self.initUI()
    ...
    def initUI(self):
        ...
        """)
        self.folder_depth_spinbox.setToolTip("Sets the number of folder levels to move up from the script's location to find the main shot folder")
        ...
        copy_to_label.setStyleSheet("font-weight: bold")  # Establecer el texto en negrita
        ...
        while True:
            ...
            if button_text_key not in self.settings_data or path_key not in self.settings_data:
            ...
            button_name_line_edit.setToolTip("Name to be displayed in the Copy menu.\nUse & before a letter in the name to use that letter as a shortcut.\ne.g., &Assets will have shortcut Alt+A")
            button_name_line_edit.setFixedWidth(283)  # Ancho fijo para la caja de entrada del nombre
            ...
            path_line_edit.setToolTip("This is the path to the folder relative to the shot's base path")
    ...
    def save_settings(self):
        ...
        new_settings_data = {'Folder scan depth': self.settings_data['Folder scan depth']}
        for i, path_layout in enumerate(self.path_layouts):
            ...
            new_settings_data[f'copy_{i + 1}_button_text'] = button_name_line_edit.text()
    ...
    def add_path(self):
        ...
        button_name_line_edit.setToolTip("Name to be displayed in the Copy menu. Use & before a letter in the name to use that letter as a shortcut (e.g., &Assets will have shortcut Alt+A)")
        button_name_line_edit.setFixedWidth(283)  # Ancho fijo para la caja de entrada del nombre
        button_name_line_edit.setStyleSheet("""
            QLineEdit {
                background-color: #282828;
    ...
    def remove_path(self, index):
        ...
        for i, path_layout in enumerate(self.path_layouts):
            ...
            new_settings_data[f'copy_{i + 1}_button_text'] = self.settings_data.get(f'copy_{i + 2}_button_text', "")
            new_settings_data[f'copy_{i + 1}_subdirectory'] = self.settings_data.get(f'copy_{i + 2}_subdirectory', "")
    ...
    def print_layout_widget_info(self):
        ...
        for i in range(count):
            item = self.layout.itemAt(i)
            if item.widget():
                debug_print(f"    Widget: {item.widget().__class__.__name__} - ObjectName: {item.widget().objectName()}")
            elif item.layout():
                debug_print(f"    Layout: {item.layout().__class__.__name__} - ObjectName: {item.layout().objectName()}")
            else:
    ...
    def format_ini_output(self):
        ...
        while True:
            ...
            ini_representation += f"copy_{current_index}_button_text = {button_text_value}\n"
            ini_representation += f"copy_{current_index}_subdirectory = {path_value}\n\n"
...
class FileScanner(QWidget):
    ...
    def initUI(self):
        ...
        for button_text, subdirectory in self.copy_options:
            ...
            # Conectar la accion
            action.triggered.connect(lambda checked=False, subdir=subdirectory: self.copy_to(subdir))
        ...
        left_buttons_layout.setContentsMargins(0, 9, 0, 9)  # Anade un margen superior e inferior de 9 pixeles
        ...
        self.settings_button.pressed.connect(lambda: self.settings_button.setIcon(settings_on_icon))
        self.settings_button.released.connect(lambda: [self.settings_button.setIcon(settings_off_icon), self.settings_button.clearFocus()])
            ...
            if 'CopyOptions' in config:
                for key in config['CopyOptions']:
                    if key.endswith('_button_text'):
                        ...
                        subdirectory = config['CopyOptions'].get(f'copy_{index}_subdirectory').strip('"')
    ...
    def show_settings_window(self):
        ...
        self.settings_window.setGeometry(
            QStyle.alignedRect(Qt.LeftToRight, Qt.AlignCenter, self.settings_window.size(), QApplication.primaryScreen().availableGeometry())
        )
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                if file.lower() == file_name_only:
                    ...
                    nuke.executeInMainThread(lambda: logger.debug(f"Archivo encontrado: {new_file_path}"))
    ...
    def delete_selected(self):
        ...
        for item in selected_items:
            ...
            read_node_name = self.table.item(row, 1).text()  # Obtiene el nombre del nodo Read de la fila seleccionada
        ...
        if multiple_delete:
            ...
            msgBox.setText(f"Are you sure you want to delete <font color='white'>{total_files_to_delete} files</font>?")
            ...
            sequence_status = self.table.item(files_to_delete[0][1], 4).text()  # Usando el indice 4 directamente
            ...
            if sequence_status.lower() == 'true':
                ...
                msgBox.setText(f"Are you sure you want to delete the <font color='white'>{total_files_to_delete} files</font> in the sequence?")
            else:
                # Si es un solo archivo, mensaje estandar
                msgBox.setText(f"Are you sure you want to send to trash <font color='white'>1 file</font>?")
    ...
    def calculate_total_files_to_delete(self, files_to_delete):
        ...
        for file_path, row in files_to_delete:
            ...
            if frame_range_match:
                ...
                debug_print(f"  - Secuencia detectada: {end_frame - start_frame + 1} archivos")
            else:
    ...
    def show_delete_window(self, total_files_to_delete):
        ...
        self.delete_window.setWindowFlags(Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint)
        self.delete_window.setStyleSheet("background-color: #333; color: white; font-weight: bold;")
        ...
        layout.setContentsMargins(20, 30, 20, 30)  # Margenes: izquierda, arriba, derecha, abajo
        ...
        lines_of_text = message.count('<br>') + 1  # Cuenta los <br> y suma 1 para considerar todas las lineas

        # Ajusta la altura de la ventana en funcion del numero de lineas
        additional_height = 40 + (lines_of_text * 10)  # Ajusta 10 pixeles adicionales por linea

        # Calcular el tamano de la ventana segun el texto
        label.adjustSize()
        self.delete_window.resize(label.sizeHint().width() + 100, label.sizeHint().height() + additional_height)
    ...
    def copy_to(self, subdirectory):
        ...
        if selected_items:
            ...
            if read_node_name != "-":
                ...
                nuke.executeInMainThread(lambda: logger.debug("\n  El footage no pertenece a ningun nodo Read."))
    ...
    def on_copy_finished(self, specific_dest_folder):
        if self.current_read_node_name:
            ...
            if read_node:
                ...
                nuke.executeInMainThread(lambda: self.update_read_node_and_gui(read_node, specific_dest_folder))
        self.loading_window.stop()
    ...
    def on_copy_finished_unico(self, specific_dest_folder=None):
        if specific_dest_folder and self.current_read_node_name:
            ...
            if read_node:
                # Ejecutar en el hilo principal
                nuke.executeInMainThread(lambda: self.update_read_node_and_gui_unico(read_node, specific_dest_folder))
        self.loading_window.stop()
    ...
    # Dialogo de confirmacion para la sobreescritura de secuencias de cuadros
        ...
        if reply == QMessageBox.Yes:
            ...
            nuke.executeInMainThread(lambda: logger.debug("  Copy operation cancelled.")) 
            self.copy_thread.copyCancelledUnico.emit()  # Emite la senal de cancelacion especifica para archivos unicos
...
class CopyThread(QThread):
    ...
    def copy_sequence(self, start_frame, end_frame, file_base, frame_padding, extension, specific_dest_folder):
        for frame in range(start_frame, end_frame + 1):
            ...
            try:
                nuke.executeInMainThread(lambda: logger.debug(f"      Copied seq sobreescribir: {frame_file} to {dest_file_path}"))
                shutil.copy(frame_file, dest_file_path)
            except Exception as e:
                nuke.executeInMainThread(lambda: logger.debug(f"      ++++Error copying: {e}"))
    ...
    def run(self):
        ...
        nuke.executeInMainThread(lambda: logger.debug(f"  copy execution time start: {start_copy_time} seconds"))
        ...
        if '#' in normalized_source_path:
            ...
            if not os.path.exists(specific_dest_folder):
                ...
                nuke.executeInMainThread(lambda: logger.debug(f"    Creating folder {specific_dest_folder}"))
            ...
            if frame_range_match:
                ...
                for frame in range(start_frame, end_frame + 1):
                    frame_file = f"{file_base.replace('#' * frame_padding, str(frame).zfill(frame_padding))}{extension}"
                    dest_file_path = os.path.join(specific_dest_folder, os.path.basename(frame_file))
                    nuke.executeInMainThread(lambda: logger.debug(f"      Copied seq unico: {frame_file} to {dest_file_path}"))
                    #try:
            ...
            if os.path.exists(normalized_source_path):
                ...
                nuke.executeInMainThread(lambda: logger.debug(f"        normalized_source_path: {normalized_source_path}"))
                dest_file_path = os.path.join(self.specific_dest_folder, os.path.basename(normalized_source_path))
                nuke.executeInMainThread(lambda: logger.debug(f"        Checking if FILE dest_file_path {dest_file_path} exists in destination..."))
        ...
        end_time = time.time()
        nuke.executeInMainThread(lambda: logger.debug(f"      Copied {copied_files_count} files in {end_time - start_copy_time} seconds."))
