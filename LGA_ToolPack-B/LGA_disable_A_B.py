...
________________________________________________________________________________
  ...
  LGA_disable_A_B v1.6 | 2024 | Lega  
...
class SelectedNodeInfo(QWidget):
    def __init__(self, nodes, initial_connections=None, disable_ab_node=None, parent=None):
        ...
        nodes = [node for node in nodes if node.Class() != "Dot" and 'disable' in node.knobs()]
    ...
    def disconnect_nodes(self):
        ...
        try:
            ...
            initial_states = self.disable_ab_node['initial_disable_states'].value().split(",")
            for state in initial_states:
                ...
                if node:
                    ...
                    debug_print(f"Node {node_name} restore disable state to {initial_value}")
                ...
                if col < 2:  # Las primeras dos columnas son checkboxes
                    ...
                    if self.table.rowCount() == 2:
                        ...
                        if col == 0 and self.initial_connections[row] == "A":
                            checkbox.setChecked(True)
                            debug_print(f"Row {row} - Column A - Node {node.name()} is checked")
                        elif col == 1 and self.initial_connections[row] == "B":
                            checkbox.setChecked(True)
                            debug_print(f"Row {row} - Column B - Node {node.name()} is checked")
    ...
    def adjust_window_size(self):
        ...
        width = self.table.verticalHeader().width() - 30  # Un poco de relleno para estetica
    ...
    def connect_nodes(self):
        ...
        try:
            ...
            if self.disable_ab_node:
                ...
                initial_disable_states = disable_ab_node['initial_disable_states'].value().split(",")
            else:
            ...
            for row in range(self.table.rowCount()):
                ...
                if self.table.cellWidget(row, 0).isChecked():  # Nodo del grupo A
                    ...
                    initial_state = next((state.split(":")[1] for state in initial_disable_states if state.split(":")[0] == node_name), None)
                    if initial_state is not None:
                        node['disable'].clearAnimated()
                        node['disable'].setValue(int(initial_state))
                        debug_print(f"Node {node_name} has no A or B selected, restoring initial state to {initial_state}")
            ...
            disable_ab_node['initial_disable_states'].setValue(",".join(initial_disable_states))
...
def main():
    ...
    selected_nodes = [node for node in selected_nodes if node.Class() != "Dot" and 'disable' in node.knobs()]
    
    # Verificar si hay un solo nodo seleccionado
    if len(selected_nodes) == 1:
        single_node = selected_nodes[0]
        if single_node.Class() == "NoOp" and single_node.name().startswith("Disable_A_B"):
