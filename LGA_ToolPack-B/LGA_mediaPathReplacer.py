...
_______________________________________________
  ...
  LGA_mediaPathReplacer v1.6 | 2024 | Lega  
...
from PySide2.QtWidgets import QSpacerItem, QSizePolicy, QTableWidget, QTableWidgetItem, QDialog, QVBoxLayout, QHeaderView, QFrame
...
class SearchAndReplaceWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.filter_checkbox.setToolTip("Toggle to enable/disable filtering of the list based on the search term.")
        ...
        divisor = QFrame()
        divisor.setFrameShape(QFrame.VLine)  # Establecer la forma del marco como una linea vertical
        divisor.setFrameShadow(QFrame.Sunken)  # Establecer la sombra del marco para un efecto 3D
        divisor.setStyleSheet("border: 1px solid #1f1f1f;")  # Ajustar el color y el grosor del borde
    ...
    def loadPresets(self):
        ...
        self.replace_preset = self.config.get('Presets', 'Replace_Preset_1', fallback='')
    ...
    def savePreset(self):
        ...
        if search_text and replace_text:
            self.config.read(self.ini_path)  # Recargar la configuracion para obtener el conteo actualizado
            preset_number = len(self.config.items('Presets')) // 2 + 1  # Calcular el nuevo numero de preset
    ...
    def replacePaths(self):
        ...
        try:
            find_text = self.find_input.text().lower()  # Version normalizada para busqueda
                ...
                if (node.Class() == 'Read' and self.read_checkbox.isChecked()) or \
                   (node.Class() == 'Write' and self.write_checkbox.isChecked()):
                    ...
                    if find_text in normalized_path:
                        ...
                        new_path = original_path[:start] + replace_text + original_path[end:]
    ...
    def updatePreviews(self):
        ...
        for node, normalized_path in zip(self.nodes, self.normalized_nodes):
            original_path = node['file'].getValue()
            if (node.Class() == 'Read' and self.read_checkbox.isChecked()) or (node.Class() == 'Write' and self.write_checkbox.isChecked()):
                ...
                if find_text in normalized_path:
                    ...
                    new_path = original_path  # Si no hay coincidencia, usar la ruta original

                # Aplicar el resaltado segun corresponda
                highlighted_original_path = original_path.replace(part_to_replace, f'<span style="color: #ff9a8a; font-weight: bold;">{part_to_replace}</span>', 1) if find_text in normalized_path else original_path
                highlighted_new_path = new_path.replace(replace_text, f'<span style="color: #ff9a8a; font-weight: bold;">{replace_text}</span>', 1) if find_text in normalized_path else new_path

                # Agregar a las vistas previas segun el estado del filtrado
                if is_filtering_enabled and find_text in normalized_path or not is_filtering_enabled:
...
class PresetsDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.setFixedHeight(min(400, self.table.rowCount() * row_height + header_height + 60))  # 60 para espacio adicional y botones
        ...
        if selected_row != -1:
            ...
            for row in range(self.table.rowCount()):
                ...
                if search_item and replace_item:
                    new_config.set('Presets', f'Search_Preset_{row + 1}', search_item.text())
                    new_config.set('Presets', f'Replace_Preset_{row + 1}', replace_item.text())

            with open(self.ini_path, 'w') as configfile:
                new_config.write(configfile)

            # Ajustar la seleccion
            new_selected_row = selected_row if selected_row < self.table.rowCount() else selected_row - 1
            if new_selected_row >= 0:
                self.table.selectRow(new_selected_row)
                self.apply_preset(self.table.currentIndex())  # Aplicar el preset de la fila seleccionada
